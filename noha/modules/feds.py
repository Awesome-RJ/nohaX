...
import noha.modules.sql.feds_sql as sql
from noha import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from noha.modules.disable import DisableAbleCommandHandler
from noha.modules.helper_funcs.alternate import send_message
from noha.modules.helper_funcs.chat_status import is_user_admin
from noha.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from noha.modules.helper_funcs.string_handling import markdown_parser
    ...
    if user.id not in DRAGONS:
        for admin in administrators:
            ...
            if status == "creator" and str(admin.user.id) != str(user.id):
                ...
                return
    if fed_id:
        message.reply_text("You cannot join 20000000 federations only 1 for one chat")
        return
