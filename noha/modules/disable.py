...
from noha import dispatcher
from noha.modules.helper_funcs.handlers import (CMD_STARTERS,
                                                        SpamChecker)
from noha.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from noha.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "noha.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "noha.modules." + args[0].rsplit(".", 1)[0]
